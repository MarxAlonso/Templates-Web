...
export default function ThreeGraph() {
  ...
  return (
    <div className="w-full h-[450px] md:h-[600px] bg-slate-50 dark:bg-[#030812] rounded-2xl md:rounded-3xl border border-slate-200 dark:border-slate-800/50 overflow-hidden relative group shadow-2xl shadow-emerald-950/10 dark:shadow-emerald-950/20">
      {/* HUD Info */}
      <div className="absolute top-4 md:top-6 left-4 md:left-8 z-10 pointer-events-none max-w-[calc(100%-2rem)]">
        <h3 className="text-emerald-600 dark:text-emerald-400 font-bold text-sm md:text-xl uppercase tracking-[0.2em] mb-1 md:mb-2 drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]">Explorador Neuronal 3D</h3>
        <p className="text-[10px] md:text-xs text-slate-600 dark:text-slate-500 font-medium">Click para navegar • Zoom para profundizar</p>
      </div>
      ...
      <div className="w-full h-full transition-opacity duration-1000">
        <ForceGraph3D
          ...
          onNodeClick={(node: any) => {
            router.push(`/nutribrain/n/${node.id}`);
          }}
          nodeResolution={24}
          linkWidth={2}
          showNavInfo={false}
